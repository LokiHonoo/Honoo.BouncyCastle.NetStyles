...
namespace Honoo.BouncyCastle
{
    ...
    public sealed class HashAlgorithmName : IEquatable<HashAlgorithmName>
    {
        ...
        internal delegate HashAlgorithm GenerateAlgorithmCallback();

        internal delegate IDigest GenerateDigestCallback();
        ...
        private readonly GenerateDigestCallback _generateDigest;
        ...
        internal GenerateDigestCallback GenerateDigest => _generateDigest;

        #endregion Properties

        #region Construction

        internal HashAlgorithmName(string name, int hashSize, GenerateDigestCallback generateDigest, GenerateAlgorithmCallback generateAlgorithm)
        {
            _name = name;
            _hashSize = hashSize;
            _generateDigest = generateDigest;
        ...
        private static bool TryGetAlgorithmNano(string mechanism, out HashAlgorithmName algorithmName)
        {
            if (mechanism.StartsWith("BLAKE2B"))
            {
                ...
                if (int.TryParse(cut, out int hashSize))
                {
                    ...
                    {
                        algorithmName = new HashAlgorithmName($"BLAKE2b{hashSize}",
                                             hashSize,
            ...
            else if (mechanism.StartsWith("BLAKE2S"))
            {
                ...
                if (int.TryParse(cut, out int hashSize))
                {
                    ...
                    {
                        algorithmName = new HashAlgorithmName($"BLAKE2s{hashSize}",
                                             hashSize,
            ...
            else if (mechanism.StartsWith("SHA512T"))
            {
                ...
                if (int.TryParse(cut, out int hashSize))
                {
                    ...
                    {
                        algorithmName = new HashAlgorithmName($"SHA512/{hashSize}",
                                             hashSize,
            ...
            else if (mechanism.StartsWith("SKEIN"))
            {
                ...
                if (splits.Length == 2)
                {
                    if (int.TryParse(splits[0], out int hashSize) && int.TryParse(splits[1], out int stateSize))
                    {
                        ...
                        {
                            algorithmName = new HashAlgorithmName($"SKEIN{hashSize}-{stateSize}",
                                                 hashSize,
